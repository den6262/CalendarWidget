...
struct MainView: View {
            ...
            VStack {
                HStack {
                    VStack(alignment: .leading) {
                        Text("Widget Plus +").font(.largeTitle).bold()
                        Text("Build your own calendar style of widget")
                            .font(.system(size: 20)).opacity(0.5)
                    }
                    Spacer()
                }.padding(20)
    ...
    private var BackgroundImgOpacity: some View {
        VStack {
            modifierSection(title: "Background Image Opacity")
            HStack {
                Text("0").foregroundColor(.secondary)
                Slider(value: $manager.alpha, in: 0.0...1.0, step: 0.1)
                    .onChange(of: manager.alpha) { (alpha) in
                        manager.updateOpacity(alpha: alpha)
                    }
